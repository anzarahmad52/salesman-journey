...
@frappe.whitelist()
def get_salesman_visit_logs_single(date=None, salesman=None, fields=None):
    """
    Get visit logs with customer details, map link, and check-in status.
    Includes ALL existing fields + new fields.
    """
    ...
    # Your valid fields list
    valid_fields = [
        "name", "customer", "salesman", "outcome", "visit_date", "next_visit_date",
        "linked_order", "customer_name", "status", "company", "territory",
        "contact_person", "contact_mobile", "remarks"
    ]
    ...
    # Fetch visit logs with requested fields
    visit_logs = frappe.get_all(
        visit_doctype,
        fields=fields,
        filters=filters,
    ...
    # --------- Add Customer Details + Map + Check-in Status ----------- #
    for log in visit_logs:
        cust_id = log.get("customer")
        customer = frappe.db.get_value(
            "Customer",
            cust_id,
            ["customer_name", "zatca_customer_name_in_arabic", "custom_latitude", "custom__longitude"],
            as_dict=True
        ) or {}

        # Include customer details
        log["customer_name"] = customer.get("customer_name")
        log["zatca_customer_name_in_arabic"] = customer.get("zatca_customer_name_in_arabic")
        log["latitude"] = customer.get("custom_latitude")
        log["longitude"] = customer.get("custom__longitude")

        # Map link
        if customer.get("custom_latitude") and customer.get("custom__longitude"):
            lat = customer.get("custom_latitude")
            lng = customer.get("custom__longitude")
            log["map_link"] = f"https://maps.google.com/?q={lat},{lng}"
        else:
            log["map_link"] = None

        # Check-in status using your existing function
        try:
            log["check_in_status"] = get_checkin_status(log.get("name"))
        except Exception:
            log["check_in_status"] = "Not Checked In"
...
def _date_range_from_filter(filter=None, from_date=None, to_date=None):
    ...
    if filter.lower() == 'today':
        date_range['from_date'] = today_date
        date_range['to_date'] = today_date
    elif filter.lower() == 'this week':
        date_range['from_date'] = today_date - timedelta(days=today_date.weekday())
        date_range['to_date'] = date_range['from_date'] + timedelta(days=6)
    elif filter.lower() == 'this month':
        date_range['from_date'] = get_first_day(today_date)
        date_range['to_date'] = get_last_day(today_date)
    elif filter.lower() == 'this year':
        date_range['from_date'] = getdate(f"{today_date.year}-01-01")
        date_range['to_date'] = getdate(f"{today_date.year}-12-31")
    elif filter.lower() == 'custom' and from_date and to_date:
        date_range['from_date'] = getdate(from_date)
        date_range['to_date'] = getdate(to_date)
    else:
        frappe.throw(_("Invalid date filter or missing date range"))
        
    return date_range

@frappe.whitelist()
def get_financial_data(date=None, salesman=None):
    """Get financial data for a specific date and salesman.
    
    Returns:
        dict: {"message": [invoice_dict, ...]}
    """
    from datetime import timedelta
    ...
    # Get all sales invoices for the date and salesman
    invoices = frappe.get_all("Sales Invoice",
        ...
        fields=[
            ...
            "status", "is_return", "due_date", "outstanding_amount", "is_pos"
        ],
    ...
    for inv in invoices:
        # Get payment modes for this invoice
        payment_modes = frappe.db.sql_list("""
            SELECT DISTINCT LOWER(mode_of_payment) 
            FROM `tabSales Invoice Payment` 
            WHERE parent = %s
        """, inv.name)
        
        # Determine payment type
        payment_type = "Temp Credit"
        if inv.is_pos:
            if any("mada" in pm for pm in payment_modes):
                payment_type = "MADA"
            else:
                payment_type = "Cash"
        ...
        invoice_data = {
            ...
            "status": inv.status,
            "is_return": 1 if inv.is_return else 0,
            "due_date": str(inv.due_date) if inv.due_date else str(inv.posting_date),
            "outstanding_amount": flt(inv.outstanding_amount),
            "is_pos": 1 if inv.is_pos else 0,
            "payment_type": payment_type,
            "payments": [],
            "payment_modes": payment_modes if payment_modes else ["Not Specified"]
        }
        
        result["message"].append(invoice_data)
    
    return result

@frappe.whitelist()
def get_salesman_closing_vouchers(salesman_user=None, from_date=None, to_date=None, status=None):
    """
    Get Sales Invoice Closing Vouchers for a specific salesman.
    
    Args:
        salesman_user (str): Email of the salesman (default: current user)
        from_date (str): Start date in YYYY-MM-DD format (default: start of current month)
        to_date (str): End date in YYYY-MM-DD format (default: today)
        status (str): Filter by closing status (Draft/Approved/Closed)
        
    Returns:
        list: List of closing vouchers with details
        [
            {
                "name": "SICV-2025-00001",
                "posting_date": "2025-10-22",
                "salesman": "user@example.com",
                "warehouse": "Warehouse Name",
                ...
                "invoices": [
                    {
                        ...
                        "customer": "CUST-001",
                        "posting_datetime": "2025-10-22 14:30:00",
                        "grand_total": 5000.0,
    ...
    if not from_date:
        ...
        from_date = getdate(from_date).strftime('%Y-%m-%d')
        
    if not to_date:
        to_date = today()
    else:
        to_date = getdate(to_date).strftime('%Y-%m-%d')
    
    # Prepare filters
    filters = {
        "docstatus": 1,  # Only submitted vouchers
        "salesman": salesman_user,
        "posting_date": ["between", [from_date, to_date]]
    }
    
    # Add status filter if provided
    if status and status in ["Draft", "Approved", "Closed"]:
        filters["closing_status"] = status
    
    # Get all closing vouchers for the salesman in date range
    closing_vouchers = frappe.get_all(
        "Sales Invoice Closing Voucher",
        filters=filters,
        fields=[
            "name",
            "posting_date",
    ...
    for voucher in closing_vouchers:
        ...
        # Format the voucher data
        voucher_data = {
            "name": voucher.name,
            "posting_date": str(voucher.posting_date),
            "from_time": str(voucher.from_time) if voucher.from_time else None,
            "to_time": str(voucher.to_time) if voucher.to_time else None,
            "salesman": voucher.salesman,
            "warehouse": voucher.warehouse,
            "company": voucher.company,
            "currency": voucher.currency,
            "total_sales": float(voucher.total_sales or 0),
            "total_tax": float(voucher.total_tax or 0),
            "total_discount": float(voucher.total_discount or 0),
            "total_returns": float(voucher.total_returns or 0),
            "total_cash": float(voucher.total_cash or 0),
            "total_bank": float(voucher.total_bank or 0),
            "total_credit": float(voucher.total_credit or 0),
            "difference_amount": float(voucher.difference_amount or 0),
            "closing_status": voucher.closing_status,
            "invoices": []
        }
        
        # Add invoice details
        for inv in invoice_rows:
            voucher_data["invoices"].append({
                "sales_invoice": inv.sales_invoice,
                "customer": inv.customer,
                "posting_datetime": str(inv.posting_datetime) if inv.posting_datetime else None,
                "grand_total": float(inv.grand_total or 0),
                "rounded_total": float(inv.rounded_total or 0),
...
@frappe.whitelist()
def submit_salesman_closing_voucher(name, salesman=None, posting_date=None, **kwargs):
    doc = frappe.get_doc("Sales Invoice Closing Voucher", name)
    if doc.docstatus == 0:  # Draft
        doc.submit()
        return {"message": "Submitted"}
    else:
        return {"message": f"Already {doc.closing_status}"}
...
# # This Method is created for backward compatibility by Anzar Ahmed
# # @frappe.whitelist(allow_guest=False)
# # def create_sales_order(**kwargs):
# #     try:
# #         # --- Handle both JSON and form-data payloads ---
...
# # This method is created for backward compatibility by Mohammed Rehan
# # @frappe.whitelist()
# # def create_sales_order(**kwargs):
# #     try:
# #         customer = kwargs.get("customer")
...
# #             # Do NOT fetch or set price here
# #             # ERPNext will apply pricing rules automatically
# #             valid_items.append({
# #                 "item_code": item_code,
# #                 "qty": qty,
# #                 "rate": price,
...
# #         # 💥 REQUIRED FOR ERPNext to calculate price correctly
# #         doc.selling_price_list = "Standard Selling"
# #         doc.currency = "SAR"
# #         doc.apply_discount_on = "Net Total"
# #         doc.ignore_pricing_rule = 0  # make sure pricing rules are applied

# #         # Add Items (ERPNext will auto-set price)
# #         for it in valid_items:
# #             doc.append("items", it)
...
# #         # Auto-calculations: prices, pricing rules, totals
# #         doc.calculate_taxes_and_totals()
...
# #             supervisor_warehouses = get_permitted_documents(
# #                 "Warehouse",
# #                 user=current_user
# #             ) or []
...
# #                 supervisor_warehouses = None

# #             if supervisor_warehouses:
# #                 if wh_list:
# #                     wh_list = list(set(wh_list) & set(supervisor_warehouses))
# #                 else:
# #                     wh_list = supervisor_warehouses
...
# #                         "page": cint(page),
# #                         "page_len": cint(page_len),
# #                         "message": "No permitted warehouses for this user"
# #                     }

# #         except Exception as e:
# #             frappe.log_error(
# #                 f"Error fetching supervisor warehouse permissions: {str(e)}"
# #             )
# #             return {
# #                 "total": 0,
# #                 "rows": [],
...
# @frappe.whitelist()
# def get_salesman_closing_vouchers(salesman_user=None, from_date=None, to_date=None, status=None, include_draft=False):
#     """
#     Get Sales Invoice Closing Vouchers for a specific salesman with all related data.
...
#         to_date (str): End date in YYYY-MM-DD format (default: today)
#         status (str): Filter by closing status (Draft/Approved/Closed)
#         include_draft (bool): Include draft (docstatus=0) vouchers (default: False)
        
#     Returns:
#         list: List of closing vouchers with complete details
#     """
#     # Set default values
#     if not salesman_user:
#         salesman_user = frappe.session.user
...
#     result = []
#     for voucher in closing_vouchers:
#         # Get cash invoice details
#         cash_invoices = frappe.get_all(
#             "SICV Invoice Row",  # Update this to your actual child table name
#             filters={"parent": voucher.name},
#             fields=[
#                 "name",
#                 "sales_invoice",
...
#         # Get bank invoice details
#         bank_invoices = frappe.get_all(
#             "SICV Invoice Row",  # Update this to your actual child table name
#             filters={"parent": voucher.name},
#             fields=[
#                 "name",
...
#             "SICV Payment Row",  # Update this to your actual child table name
#             filters={"parent": voucher.name},
#             fields=[
#                 "name",
#                 "mode_of_payment",
...
#         # Get payment entries details
#         payment_entries = frappe.get_all(
#             "SICV Payment Entry",  # Update this to your actual child table name
#             filters={"parent": voucher.name},
#             fields=[
#                 "name",
#                 "payment_entry",
...
#         # Initialize accounting_entries and journal_entry_details
#         accounting_entries = []
#         journal_entry_details = {}
        
#         # Get accounting entries (Journal Entry details) if journal_entry exists
#         if voucher.journal_entry:
#             try:
#                 je = frappe.get_doc("Journal Entry", voucher.journal_entry)
#                 if je:
#                     for acc in je.accounts:
#                         accounting_entries.append({
#                             "account": acc.account,
#                             "account_name": frappe.db.get_value("Account", acc.account, "account_name"),
#                             "debit": float(acc.debit_in_account_currency or 0),
#                             "credit": float(acc.credit_in_account_currency or 0),
#                             "user_remark": acc.user_remark or "",
#                             "against_account": acc.against_account or "",
#                             "party_type": acc.party_type or "",
#                             "party": acc.party or ""
#                         })
                    
#                     # Add journal entry header info
#                     journal_entry_details = {
#                         "name": je.name,
...
#                         "difference": float(je.difference or 0),
#                         "user_remark": je.user_remark or "",
#                         "remark": je.remark or ""
#                     }
#             except Exception as e:
#                 # Journal Entry might be deleted or not found
#                 frappe.log_error(f"Error fetching Journal Entry {voucher.journal_entry}: {str(e)}")
...
#             "to_time": str(voucher.to_time) if voucher.to_time else None,
#             "salesman": voucher.salesman,
#             "salesman_name": frappe.db.get_value("User", voucher.salesman, "full_name") if voucher.salesman else "",
#             "warehouse": voucher.warehouse,
#             "warehouse_name": frappe.db.get_value("Warehouse", voucher.warehouse, "warehouse_name") if voucher.warehouse else "",
#             "company": voucher.company,
#             "currency": voucher.currency,
#             "total_sales": float(voucher.total_sales or 0),
#             "total_returns": float(voucher.total_returns or 0),
...
#             "creation": str(voucher.creation),
#             "modified": str(voucher.modified),
#             "cash_invoices": [],
#             "bank_invoices": [],
#             "payment_summary": [],
#             "payment_entries": [],
#             "accounting_entries": accounting_entries,
#             "journal_entry_details": journal_entry_details if journal_entry_details else None
#         }
        
#         # Add cash invoice details
#         for inv in cash_invoices:
#             voucher_data["cash_invoices"].append({
#                 "row_id": inv.name,
#                 "sales_invoice": inv.sales_invoice,
#                 "customer": inv.customer,
#                 "customer_name": frappe.db.get_value("Customer", inv.customer, "customer_name") if inv.customer else "",
#                 "posting_datetime": str(inv.posting_datetime) if inv.posting_datetime else None,
#                 "grand_total": float(inv.grand_total or 0),
#                 "outstanding_amount": float(inv.outstanding_amount or 0),
#                 "mode_of_payment": inv.mode_of_payment,
#                 "mode_of_payment_name": frappe.db.get_value("Mode of Payment", inv.mode_of_payment, "mode_of_payment") if inv.mode_of_payment else "",
#                 "is_return": 1 if inv.is_return else 0,
#                 "idx": inv.idx
#             })
        
#         # Add bank invoice details
#         for inv in bank_invoices:
#             voucher_data["bank_invoices"].append({
#                 "row_id": inv.name,
#                 "sales_invoice": inv.sales_invoice,
#                 "customer": inv.customer,
#                 "customer_name": frappe.db.get_value("Customer", inv.customer, "customer_name") if inv.customer else "",
...
#         # Add payment summary details
#         for ps in payment_summary:
#             voucher_data["payment_summary"].append({
#                 "row_id": ps.name,
#                 "mode_of_payment": ps.mode_of_payment,
#                 "mode_of_payment_name": frappe.db.get_value("Mode of Payment", ps.mode_of_payment, "mode_of_payment") if ps.mode_of_payment else "",
#                 "amount": float(ps.amount or 0),
#                 "idx": ps.idx
#             })
        
#         # Add payment entries details
#         for pe in payment_entries:
#             voucher_data["payment_entries"].append({
#                 "row_id": pe.name,
#                 "payment_entry": pe.payment_entry,
#                 "posting_datetime": str(pe.posting_datetime) if pe.posting_datetime else None,
#                 "mode_of_payment": pe.mode_of_payment,
#                 "mode_of_payment_name": frappe.db.get_value("Mode of Payment", pe.mode_of_payment, "mode_of_payment") if pe.mode_of_payment else "",
#                 "party_type": pe.party_type,
#                 "party": pe.party,
#                 "party_name": frappe.db.get_value(pe.party_type, pe.party, "customer_name" if pe.party_type == "Customer" else "name") if pe.party_type and pe.party else "",
...
# @frappe.whitelist()
# def get_salesman_closing_summary(salesman_user=None, from_date=None, to_date=None):
#     """
#     Get summary statistics for salesman closing vouchers.
...
#             "total_bank": 35000.0,
#             "net_sales": 95000.0,
#             "average_daily_sales": 9500.0,
#             "vouchers_by_status": {"Draft": 1, "Closed": 9},
#             "vouchers_by_warehouse": {"Warehouse A": 5, "Warehouse B": 5}
#         }
#     """
#     # Set default values
#     if not salesman_user:
...
#     # Get all vouchers for summary
#     vouchers = get_salesman_closing_vouchers(
#         salesman_user=salesman_user,
#         from_date=from_date,
#         to_date=to_date,
#         include_draft=True
...
#             "vouchers_by_status": {},
#             "vouchers_by_warehouse": {},
#             "date_range": {"from_date": from_date, "to_date": to_date}
#         }
    
#     # Calculate statistics
#     total_vouchers = len(vouchers)
#     total_sales = sum(v.get("total_sales", 0) for v in vouchers)
#     total_returns = sum(v.get("total_returns", 0) for v in vouchers)
#     total_cash = sum(v.get("total_cash", 0) for v in vouchers)
#     total_bank = sum(v.get("total_bank", 0) for v in vouchers)
#     net_sales = total_sales - total_returns
    
#     # Calculate average daily sales (only for days with vouchers)
#     unique_dates = set(v.get("posting_date") for v in vouchers if v.get("posting_date"))
#     days_count = len(unique_dates)
#     average_daily_sales = net_sales / days_count if days_count > 0 else 0
    
#     # Count by status
#     vouchers_by_status = {}
#     for v in vouchers:
#         status = v.get("closing_status", "Draft")
#         vouchers_by_status[status] = vouchers_by_status.get(status, 0) + 1
    
#     # Count by warehouse
#     vouchers_by_warehouse = {}
#     for v in vouchers:
#         warehouse = v.get("warehouse_name") or v.get("warehouse") or "Unknown"
#         vouchers_by_warehouse[warehouse] = vouchers_by_warehouse.get(warehouse, 0) + 1
...
#         "vouchers_by_warehouse": vouchers_by_warehouse,
#         "date_range": {
#             "from_date": from_date,
#             "to_date": to_date,
#             "days": days_count
...
# # @frappe.whitelist()
# # def approve_closing_voucher(voucher_name):
# #     """Sales user approves the voucher"""
# #     try:
# #         doc = frappe.get_doc("Sales Invoice Closing Voucher", voucher_name)
        
# #         # Check if user is the salesman
# #         if doc.salesman != frappe.session.user:
# #             frappe.throw("You can only approve your own vouchers")
        
# #         # Check workflow state
# #         if doc.workflow_state != "Pending for Salesman Approval":
# #             frappe.throw("Voucher is not in pending approval state")
        
# #         # Update workflow state and submit
# #         frappe.db.set_value("Sales Invoice Closing Voucher", voucher_name, {
# #             "workflow_state": "Approved",
# #             "custom_approved_by": frappe.session.user,
# #             "custom_approved_date": frappe.utils.now_datetime()
# #         })
        
# #         doc.submit()
# #         frappe.db.commit()
        
# #         return {"message": {"success": True, "message": "Voucher approved"}}
        
# #     except Exception as e:
# #         frappe.log_error(f"Approve failed: {str(e)}")
# #         return {"error": str(e)}

# # @frappe.whitelist()
# # def request_modification(voucher_name, reasons):
# #     """Sales user requests modification"""
# #     try:
# #         doc = frappe.get_doc("Sales Invoice Closing Voucher", voucher_name)
        
# #         # Check if user is the salesman
# #         if doc.salesman != frappe.session.user:
# #             frappe.throw("You can only request modification for your own vouchers")
        
# #         # Check workflow state
# #         if doc.workflow_state != "Pending for Salesman Approval":
# #             frappe.throw("Voucher is not in pending approval state")
        
# #         # Update to modification requested state
# #         frappe.db.set_value("Sales Invoice Closing Voucher", voucher_name, {
# #             "workflow_state": "Modify Request By Salesman",
# #             "custom_modification_reasons": reasons,
# #             "custom_modification_date": frappe.utils.now_datetime()
# #         })
        
# #         frappe.db.commit()
        
# #         return {"message": {"success": True, "message": "Modification requested"}}
        
# #     except Exception as e:
# #         frappe.log_error(f"Modification request failed: {str(e)}")
# #         return {"error": str(e)}

# @frappe.whitelist()
# def approve_or_submit_closing_voucher(docname, action="submit"):
#     """
#     Approve or submit a Sales Invoice Closing Voucher
    
#     Args:
#         docname (str): Name of the Sales Invoice Closing Voucher document
#         action (str): Action to perform - 'submit' or 'approve'
...
#     # Check if document exists
#     if not frappe.db.exists("Sales Invoice Closing Voucher", docname):
#         frappe.throw(_(f"Sales Invoice Closing Voucher {docname} does not exist."))
    
#     doc = frappe.get_doc("Sales Invoice Closing Voucher", docname)
    
#     # Check permissions - ensure user is the salesman
#     if doc.salesman != frappe.session.user:
#         frappe.throw(_("You can only approve/submit your own closing vouchers."))
...
#                 # Update workflow state to Pending for Salesman Approval
#                 frappe.db.set_value("Sales Invoice Closing Voucher", docname, {
#                     "workflow_state": "Pending for Salesman Approval"
#                 })
                
#                 # Add comment
#                 doc.add_comment("Workflow", _("Submitted by {0}").format(frappe.session.user))
                
#                 return {
#                     "status": "success",
#                     "message": _("Closing Voucher {0} has been submitted successfully").format(docname),
#                     "docstatus": doc.docstatus
#                 }
#             except Exception as e:
#                 frappe.log_error(frappe.get_traceback(), "Closing Voucher Submit Error")
#                 frappe.throw(_(f"Error submitting Closing Voucher: {str(e)}"))
#         else:
#             frappe.throw(_("Only draft Closing Vouchers can be submitted."))
...
#                 # Check workflow state
#                 if doc.workflow_state != "Pending for Salesman Approval":
#                     frappe.throw(_("Cannot approve. Voucher is not in 'Pending for Salesman Approval' state."))
...
#                 return {
#                     "status": "success",
#                     "message": _("Closing Voucher {0} has been approved successfully").format(docname),
#                     "docstatus": doc.docstatus
#                 }
                
#             except Exception as e:
#                 frappe.log_error(frappe.get_traceback(), "Closing Voucher Approval Error")
#                 frappe.throw(_(f"Error approving Closing Voucher: {str(e)}"))
#         else:
#             frappe.throw(_("Only submitted Closing Vouchers can be approved."))
...
# @frappe.whitelist()
# def request_voucher_modification(docname, reasons):
#     """
#     Request modification for a Sales Invoice Closing Voucher
    
#     Args:
#         docname (str): Name of the Sales Invoice Closing Voucher document
#         reasons (str): Reasons for modification
...
#     # Check permissions - ensure user is the salesman
#     if doc.salesman != frappe.session.user:
#         frappe.throw(_("You can only request modification for your own closing vouchers."))
    
#     # Check if voucher is in correct state
#     if doc.docstatus != 1 or doc.workflow_state != "Pending for Salesman Approval":
#         frappe.throw(_("Modification can only be requested for submitted vouchers in 'Pending for Salesman Approval' state."))
    
#     try:
#         # Update workflow state to Modify Request By Salesman
#         frappe.db.set_value("Sales Invoice Closing Voucher", docname, {
#             "workflow_state": "Modify Request By Salesman",
#             "custom_modification_reasons": reasons,
#             "custom_modification_requested_by": frappe.session.user,
#             "custom_modification_date": frappe.utils.now_datetime()
#         })
        
#         # Add comment
#         doc.add_comment("Workflow", 
#             _("Modification requested by {0}. Reasons: {1}").format(frappe.session.user, reasons))
        
#         return {
#             "status": "success",
#             "message": _("Modification request submitted for Closing Voucher {0}").format(docname)
#         }
